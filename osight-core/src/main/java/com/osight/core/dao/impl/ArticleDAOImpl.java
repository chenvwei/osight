...
public class ArticleDAOImpl extends BaseHibernateDAO implements ArticleDAO {
    ...
    public Page<ArticleData> getArticles(int start, int count, boolean visible) {
        if (visible) {
            ...
            List<ArticleData> list = hibernateUtil.find("select p from ArticleData p where p.secret=false order by p.createdOn desc", null, null,
                    start, count);
            long ct = hibernateUtil.getCount("select p from ArticleData p where p.secret=false", null, null);
            Page<ArticleData> page = PageUtil.getPage(list.iterator(), start, count, ct);
            return page;
        }
    }
    ...
    public Page<ArticleCommentData> getCommentsByArticleId(long articleId, int start, int count) {
        ...
        long ct = hibernateUtil.getCount("select p from ArticleCommentData p where p.articleId=?", new Object[] {articleId},
